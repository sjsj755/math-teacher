...
def build():
    ...
    add_para(
        doc,
        "修订记录：V1.8（2026-08-08）标记阶段 5 已实现（通知服务：FastAPI 服务端＋App 日报摘要与设置页），待 QQ 真机验收；"
        "V1.7（2026-08-08）标记阶段 4 已完成，记录练习与错题闭环（推荐/重做/周清、状态机）；"
        "V1.6（2026-08-08）标记阶段 3 已完成，记录诊断与报告实现（含填空/限时题、评分引擎、雷达图）；"
        "V1.5（2026-08-08）新增附录C 视觉设计规范（清新学习风·克制可爱版 v0.3.0），记录全页面视觉改版与构建环境调整；"
        "V1.4（2026-08-08）标记阶段 2 已完成，记录数据层与题库实现细节；"
    ...
    add_callout(
        doc,
        "当前进度",
        "阶段 0–4 ✅ 已完成；视觉改版（v0.3.0 清新学习风·克制可爱版）✅ 已完成；"
        "阶段 5 通知服务 ✅ 已实现（服务端 24 项＋App 51 项测试通过，待 QQ 真机验收）；下一阶段：阶段 6 联调部署。",
    )
    add_bullets(
        doc,
        [
            ...
            "✅ 已完成：视觉改版 v0.3.0（清新学习风·克制可爱版：主题令牌、共享组件、GeoSpirit 吉祥物、全页面改版），13 项测试通过，APK 已覆盖安装真机；",
            "✅ 已完成：阶段 3 诊断与报告（15 题诊断含填空/限时题、四因子评分、完成弹层、报告页雷达图），25 项测试通过；",
            "✅ 已完成：阶段 4 练习与错题闭环（推荐练习、错题重做、周清复测、状态机），40 项测试通过；",
            "✅ 已实现：阶段 5 通知服务（notify_server FastAPI：绑定码/绑定/状态/解绑/日报上送/QQ 回调/APScheduler 定时推送与失败重试；"
            "App 端日报摘要生成与离线队列上送、设置页绑定码/推送时间/日报开关/解绑与 30 秒状态轮询），"
            "服务端 24 项 pytest＋App 51 项 flutter test 全部通过，dart analyze 无问题；",
            "○ 待办：内网穿透注册（cpolar/花生壳）；阶段 6 联调部署（摘要上送→定时推送→家长 QQ 收到日报）。",
        ],
    )
    ...
    add_table(
        ...
        ["方法", "路径", "用途", "调用方"],
        [
            ["POST", "/api/v1/bind-code", "生成绑定码（24 小时有效）", "学生端"],
            ["POST", "/api/v1/bind", "学生端确认绑定码关联", "学生端"],
            ["GET", "/api/v1/status", "查询绑定与推送状态", "学生端"],
            ["POST", "/api/v1/unbind", "学生端解绑", "学生端"],
    ...
    add_table(
        ...
        [
            ...
            ["阶段3 诊断报告", "诊断页、评分引擎、报告页", "完成一次诊断并看到雷达图", "✅ 已完成", "1–2 天"],
            ["阶段4 练习错题", "练习页、计时、错题状态机", "答错自动入错题本并可重做", "✅ 已完成", "2 天"],
            ["阶段5 通知服务", "FastAPI、绑定、摘要、QQ 推送", "家长 QQ 收到测试日报", "✅ 已实现（待真机验收）", "2 天"],
    ...
    add_para(
        doc,
        "阶段 2 状态：✅ 已完成（2026-08-08，内容校验与自动化测试通过；调试 APK 已覆盖安装到 iQOO Z5x，首页题库状态待目视确认）。实际记录：",
    )
    add_bullets(
        doc,
        [
    ...
    add_para(
        doc,
        "阶段 3 状态：✅ 已完成（2026-08-08，完成一次诊断并在报告页看到雷达图，真机覆盖安装验收）。实际记录：",
    )
    add_bullets(
        doc,
        [
            "诊断 15 题：K6（年级章节基础题，含填空）＋T5（难度 3 带思维方法）＋S2（步骤自评）＋P2（1 道限时题＋1 道心态自评）；组内随机，顺序 K→T→S→P；",
            "题库新增 4 道 is_timed 限时题（content_version 2），config.json 配置诊断配比与限时（choice 120s）；",
            "DiagnosticEngine：choice 字母判分、fill 精确/小数等价判分、自评选项 0/1/2→1/0.5/0、超时计错、薄弱点（≥2 题且正确率<60% 升序）、归因（错题 lose_type 前 2，S/P 低时补规范/心理）；",
            "迁移 002：question.is_timed、answer_record.self_option、diagnosis.attribution，schema_version 2，v1 升级保留旧数据已验证；",
            "界面：LatexText 行内公式、逐题作答（进度/上一题/下一题/倒计时/填空输入）、完成弹层（开心小精灵＋得分概览）、报告页（综合得分＋fl_chart 雷达图＋薄弱点＋归因）；",
            "高二/高三题库未覆盖时自动从全库基础题补足并在完成层提示；",
            "测试：25 项全部通过（引擎 7＋装配 3＋服务集成 2＋数据层 6＋主题 4＋冒烟 3）。",
        ],
    )
    add_para(
        doc,
        "阶段 4 状态：✅ 已完成（2026-08-08，练习答错自动入错题本并可重做，真机覆盖安装验收）。实际记录：",
    )
    add_bullets(
        doc,
        [
            "练习三模式：推荐练习（薄弱点前 3＋难度 6/3/1 共 10 题）／错题重做（pending＋升级类，升级类用同类题）／周清复测（观察期到期题）；",
            "逐题限时（choice 120s / fill 180s，超时计错）＋提交后即时反馈（答案/解析/采分点，错误自动收录错题本）；",
            "错题状态机（表 10）：待重做→已重做（观察期 7 天）→已掌握；重做/复测再错→待重做（升级）且 redo_count+1；",
            "错题本页：状态筛选（全部/待重做/观察中/已掌握）、错题卡片与解析弹窗、重做/周清入口；",
            "共享组件：QuestionAnswerPanel（诊断/练习复用作答面板）、AnswerGrading（choice/fill 统一判分）；",
            "测试：40 项全部通过（新增状态机 8＋推荐 3＋服务集成 4）。",
        ],
    )
    ...
    add_para(
        doc,
        "阶段 5 状态：✅ 已实现（2026-08-08，服务端 24 项 pytest＋App 51 项 flutter test 全部通过，"
        "dart analyze 无问题；家长 QQ 收到日报需接入 QQ 凭据后真机验收）。实际记录：",
    )
    add_bullets(
        doc,
        [
            "notify_server/（FastAPI＋SQLite＋APScheduler，DDD 分层）：config.py 环境变量配置（QQ 凭据/通道/推送时间），"
            "domain/entities.py 绑定码与业务错误码，infrastructure/repository.py 四表（bind_codes/bindings/digest_log/push_log），"
            "channels/ 默认 mock 通道＋QQBotChannel（凭据经环境变量注入，QQ_API_BASE 可覆盖）＋EmailChannel 二线预案；",
            "接口：POST /bind-code（24 小时绑定码）、POST /bind（学生端确认，幂等）、GET /status（每 30 秒轮询）、"
            "POST /unbind、POST /daily-digest（未绑定返回 404 NOT_BOUND）、POST /qq/callback（绑定/日报/解绑，兼容官方 v2 payload）；"
            "错误码：BIND_CODE_INVALID/ALREADY_BOUND/DEVICE_MISMATCH/NOT_BOUND；",
            "application/scheduler.py：APScheduler CronTrigger 每日 21:30 推送，DigestSender 失败重试 3 次并逐次写 push_log；"
            "日报模板按 5.4.1：日期/题数/正确率/错题知识点/时长/连续打卡/明日建议（weak_point_names 缺省回退编码）；",
            "App 端：DailyDigest 模型＋DigestService（从当日 answer_record 聚合题数/正确率/错题知识点/时长/连续天数，"
            "写 digest_queue（同日覆盖）→ 有网自动上送，离线保留下次重试）；启动时与练习完成后自动生成并补传；",
            "设置页改版：服务器地址、日报开关、推送时间（TimePicker）、绑定码展示/复制/重新生成/确认绑定、"
            "已绑定卡片（家长昵称/最近推送/解绑），未绑定时每 30 秒轮询 /status 自动识别家长完成绑定；",
            "新增测试：pytest 24 项（绑定全流程/错误码/回调三指令/官方 v2 payload/摘要上送/重试 3 次）＋"
            "flutter test 11 项（摘要生成/队列覆盖/离线补传/绑定 API/设置页冒烟），全量 51 项通过；",
            "说明：Windows 下 flutter test 需 sqlite3.dll 可被加载（已加入用户 PATH：C:\\Users\\陈乔源\\sqlite3）；"
            "flutter analyze 因中文路径的 LSP 解析问题改用 dart analyze（无问题）。",
        ],
    )
    ...
    add_h1(doc, "附录C　视觉设计规范（v0.3.0 清新学习风·克制可爱版）")
    add_para(
        doc,
        "设计策略：80% 简洁＋20% 可爱。功能场景（做题、报告数据区）保持极简专业；"
        "情感场景（欢迎、完成、空状态、徽章）由 GeoSpirit 几何小精灵与可爱图标表达。"
        "所有插画与吉祥物均由 Flutter 组件/CustomPaint 代码绘制，不引入图片资源。",
    )
    add_table(
        doc,
        "表17　配色令牌",
        ["角色", "色值", "用途"],
        [
            ["主色", "#0B7E82", "按钮、选中态、标题条（白字对比度≥4.5:1 达标 AA）"],
            ["辅助蓝绿", "#0E9AA0", "大色块、渐变"],
            ["柠檬黄", "#FFC53D", "徽章与亮点，配深棕文字 #3D2E00，面积≤5%"],
            ["背景薄荷", "#F0FBF9", "页面背景"],
            ["奶油白", "#FFF9F3", "欢迎页与情感卡片"],
            ["表面白", "#FFFFFF", "卡片表面"],
            ["正文/次文字", "#1F2937 / #6B7280", "文字层级"],
            ["对/错", "#2E9E6B / #D9534F", "作答反馈（图标与标签）"],
        ],
        [1600, 2400, 5360],
        ["left", "left", "left"],
    )
    add_table(
        ...
        ["组件", "说明"],
        [
            ["AppCard", "白底圆角 20、极浅阴影、可选主色描边"],
            ["AppFilledButton / AppSoftButton", "圆角 16、按压回弹微动效"],
            ["AppBadge", "柠檬黄徽章，弹跳入场，用于亮点/打卡"],
            ["SectionHeader", "主色短条＋标题，区块信息层级"],
            ["BrandMark", "蓝绿渐变圆角方块＋π，品牌识别"],
            ["AppProgressBar", "圆角进度条，诊断/练习进度"],
            ["GeoSpirit", "几何小精灵（平静/开心/加油三表情＋公式气泡），仅情感场景，做题与报告数据区禁用"],
            ["图标", "Material Icons.*_rounded 面性圆角风格"],
        ],
        [3300, 6060],
        ["left", "left"],
    )
    add_bullets(
        doc,
        [
            "字号 12/14/16/20/24/28（2 进制栅格），字重仅 Regular/Medium/Bold，不打包自定义字体；",
            "间距统一 8 倍数（8/16/24/32），圆角 卡片 20 / 按钮 16 / 芯片输入 12；",
            "MVP 仅浅色主题，不做深色模式与动态取色，保证品牌色稳定；",
            "构建环境：pub 缓存使用纯 ASCII 路径（PUB_CACHE=D:\\pub-cache）规避中文路径原生库编译问题；sqlite3 测试依赖使用系统库（pubspec hooks source: system）。",
        ],
    )
